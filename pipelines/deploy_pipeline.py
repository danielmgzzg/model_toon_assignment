...
def deploy_pipeline():
    ...
    existing_pipeline = next((p for p in existing_pipelines if p.display_name == PIPELINE_NAME), None)
    ...
    if existing_pipeline:
        ...
        print(f"Pipeline {PIPELINE_NAME} updated successfully: {updated_pipeline.display_name}")
    else:
        # Create a new pipeline
        new_pipeline = client.upload_pipeline(pipeline_package_path=output_file, pipeline_name=PIPELINE_NAME)
        print(f"Pipeline {PIPELINE_NAME} created successfully: {updated_pipeline,display_name}")
